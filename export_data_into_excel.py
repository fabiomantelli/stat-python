...
def export_data_into_excel(status_flags, pmu, date):
    ...
    for i in range(0, len(status_flags), 2):
        ...
        binary_string = "0" * (8 - len(binary_string) % 8) + binary_string  # zero-pad to a multiple of 8 bits
        binary_list = [binary_string[i:i+4] for i in range(0, len(binary_string), 4)]
        ...
        worksheet.cell(row=new_line, column=3).value = hex(status_flags[i]['Value'])[2:].upper()
        worksheet.cell(row=new_line, column=4).value = binary_with_spaces
        
        # Convert time strings to datetime objects
        initial_time = datetime.datetime.strptime(status_flags[i]['Time'], "%Y-%m-%d %H:%M:%S.%f")
        final_time = datetime.datetime.strptime(status_flags[i+1]['Time'], "%Y-%m-%d %H:%M:%S.%f")
        
       # Calculate duration in seconds
        duration = (final_time - initial_time).total_seconds()

        worksheet.cell(row=new_line, column=5).value = initial_time.strftime("%H:%M:%S.%f")[:-3]
        worksheet.cell(row=new_line, column=6).value = final_time.strftime("%H:%M:%S.%f")[:-3]

        # Format duration as h:mm:ss.000
        hours, remainder = divmod(duration, 3600)
        minutes, seconds = divmod(remainder, 60)
        milliseconds = round((seconds - int(seconds)) * 1000)
        duration_str = "{:02d}:{:02d}:{:02d}.{:03d}".format(int(hours), int(minutes), int(seconds), milliseconds)
