...
class ExcelHandler():
    ...
    def export_data_into_excel(self, status_flags, pmu, date, server_name):
        ...
        self.header_in_excel(worksheet, pmu)
        self.configure_pmu_summary_header(worksheet)
        self.get_period_time_of_status_flags(worksheet, status_flags, date)
        TOTAL_PERIOD_COLUMN = 12
        total_duration_of_status_flags = self.get_total_duration_of_status_flags(worksheet)
        worksheet.cell(row=3, column=TOTAL_PERIOD_COLUMN).value = total_duration_of_status_flags
        STATUS_FLAGS_HEX_COLUMN = 3
        most_common_status_flags, amount_of_status_flags  = self.get_total_items_in_an_excel_column(worksheet, STATUS_FLAGS_HEX_COLUMN)
        ...
        average_period = self.get_average_period(worksheet)
        worksheet.cell(row=3, column=AVERAGE_PERIOD_COLUMN).value = average_period
        sintese_worsheet = workbook['Sintese']
        self.export_data_into_summary_header(worksheet, sintese_worsheet, pmu)
        workbook.save(excel_file_name)

    def header_in_excel(self, worksheet, pmu):
        FIRST_LINE=1
        FIRST_COLUMN=1
        LAST_COLUMN=7
    ...
    def configure_pmu_summary_header(self, worksheet):
    ...
    def get_period_time_of_status_flags(self, worksheet, status_flags, date):
    ...
    def get_total_duration_of_status_flags(self, worksheet):
        PERIOD_COLUMN = 7
    ...
    def get_total_items_in_an_excel_column(self ,worksheet, column):
        status_flags = []
    ...
    def get_average_period(self, worksheet):
        TOTAL_PERIOD_COLUMN = 12
        FREQUENCY_COLUMN = 10
    ...
    def export_data_into_summary_header(self, worksheet, sintese_worksheet, pmu):
